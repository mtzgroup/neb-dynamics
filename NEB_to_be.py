...
def spring_grad_neb(
    view, k=0.1, ideal_distance=0.5, en_func=toy_potential, grad_func=toy_grad
):
...
def update_points_spring_neb(chain, dr, en_func, grad_func, k=1, ideal_dist=0.5):
    ...
    for i in range(1, len(chain) - 1):
        ...
        dr, _ = ArmijoLineSearch(
            f=en_func,
            xk=chain[i],
            gfk=np.array([grad_x, grad_y]),
            phi0=en_func(chain[i]),
            alpha0=0.1,
            pk=-1 * np.array([grad_x, grad_y]),
        )
...
def _check_grad_converged(chain_prev, chain_new, grad_func, grad_thre):
    for i in range(
        1, len(chain_prev) - 1
    ):  # don't consider the endpoints, they're fixed
        # print(f"---->{i}")
        node_prev = chain_prev[i]
        node_new = chain_new[i]
...
def optimize_chain(
    ...
    while nsteps < max_steps and not chain_optimized:
        ...
        en_converged = _check_en_converged(
            chain_prev=chain_previous,
            chain_new=new_chain,
            en_func=en_func,
            en_thre=en_thre,
        )
        grad_converged = _check_grad_converged(
            chain_prev=chain_previous,
            chain_new=new_chain,
            grad_func=grad_func,
            grad_thre=grad_thre,
...
opt_chain_neb, _ = optimize_chain(
    chain=chain,
    en_func=en_func,
    grad_func=grad_func,
    en_thre=0.01,
    grad_thre=0.01,
    update_func=update_points_spring_neb,
    max_steps=nsteps,
    k=k,
)
...
# + tags=[]
final_chain, all_chains = optimize_chain(
    chain=chain,
    en_func=en_func,
    grad_func=grad_func,
    en_thre=0.01,
...
anim = FuncAnimation(
    fig=fig, func=animate, frames=all_chains, blit=True, repeat_delay=1000, interval=500
)

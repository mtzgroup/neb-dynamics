...
class BFGS(Optimizer):
    ...
    def bfgs_step(self, x_vector):
        ...
        gradients = self._compute_and_matmul_inv_hess(hess_prev=hess_prev, orig_grad=orig_grad)
        disp = self.linesearch(x_0=x_vector, z=-1*gradients)

        xprime = x_vector - gradients * disp
        grad_prime = self.grad_func(xprime)

        self._update_hessian(orig_gradients=orig_grad, disp=disp, grad_prime=grad_prime, gradients=gradients, hess_prev=hess_prev)

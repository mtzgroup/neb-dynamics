...
class VelocityProjectedOptimizer(Optimizer):
    ...
    zero_vel_count_thre: int = 5 # threshold for decreasing timestep if repeated velocity resetting
    
    def __post_init__(self): 
        self.zero_vel_count = 0
    
    def copy(self):
        return VelocityProjectedOptimizer(timestep=self.timestep, 
                                          activation_tol=self.activation_tol)
    ...
    def optimize_step(self, chain, chain_gradients):
        ...
        while new_chain_gradients_fails and retry_count < retry_count_max:
            try:
                ...
                print("VPO: Step size too large. Decreasing.")    
                self.timestep *= .5
                retry_count+=1

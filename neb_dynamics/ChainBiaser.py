...
class ChainBiaser:
    ...
    def node_wise_distance(self, chain):
        ...
        #### this one is a simplified frechet
            ...
            tot_dist = sum(
                [
                    self._simp_frechet_helper(coords)
                    for coords in chain.coordinates[1:-1]
                ]
            )
        else:
            raise ValueError(
                f"Invalid distance func name: {self.distance_func}. Available are: {AVAIL_DIST_FUNC_NAMES}"
            )

...
class ReactionProfileGenerator:
    ...
    def transition_state(self):
        if self._cached_ts:
            ...
            if isinstance(self.input_obj, Chain):
                ...
                raise ValueError(
                    f"Invalid input object: {type(self.input_obj)}. Must be Chain or Node"
                )
            ...
            if output.success:
                ...
                ts.update_tc_parameters(
                    ts_guess
                )  # TODO: fix this so that the params come from cc result

                freqs = ts.tc_freq_calculation()
                nma = (
                    ts.tc_nma_calculation()
                )  # these are sorted from min freq to max freq
    ...
    def create_pseudo_irc_inputs(self, dr=0.1):
        ...
        inps = [
            td_disp_plus._prepare_input("optimization"),
            td_disp_minus._prepare_input("optimization"),
        ]
        return inps
    ...
    def create_alleged_profile_inputs(self):
        ...
        inp_objs = [
            r._prepare_input(method="optimization"),
            p._prepare_input(method="optimization"),
        ]
    ...
    def profiles_are_identical(alleged_profile, pseu_irc_profile):
        ...
        simply_true = arp[0]._is_connectivity_identical(pirp[0]) and arp[
            -1
        ]._is_connectivity_identical(pirp[-1])
        flipped_but_true = arp[0]._is_connectivity_identical(pirp[-1]) and arp[
            -1
        ]._is_connectivity_identical(pirp[0])

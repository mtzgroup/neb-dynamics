...
def align_structures(structure1: Structure, structure2: Structure, distance_metric="rmsd"):
    ...
    if distance_metric.upper() == 'GEODESIC':
        dist_name = "GI_DIST"
    elif distance_metric.upper() == 'RMSD':
        dist_name = "RMSD"
    elif distance_metric == 'XTB':
        dist_name = "GI-XTB"

    # Read in original coordinates and labels of xyz1 and xyz2
    a_labels, a_coords, NA_a = structure1.symbols, structure1.geometry, len(
        structure1.symbols)
    b_labels, b_coords, NA_b = structure2.symbols, structure2.geometry, len(
        structure2.symbols)
    ...
    if NA_a == NA_b:
        ...
        if distance_metric == 'XTB':
            node1 = StructureNode(structure=structure1)
            node2 = StructureNode(structure=structure2)
            InitRMSD_unsorted = ch.calculate_geodesic_xtb_barrier(node1, node2)
        elif distance_metric == 'RMSD':
            InitRMSD_unsorted = kabsch(A_all, B_all)
    else:
        print("Error: unequal number of atoms. " +
              str(NA_a) + " is not equal to " + str(NA_b))
        sys.exit()
   ...
   """
    if InitRMSD_unsorted < 0.001:
        ...
        print(f"All-atom {dist_name}: %2.3f" % float(InitRMSD_unsorted))
        sys.exit()
   ...
   """
    a_labels, a_coords, NA_a, order = _sort_xyz(
        geometry=structure1.geometry, symbols=structure1.symbols)
    sorted_a = Structure(geometry=a_coords, symbols=a_labels)
    Uniq_a = list(set(a_labels))
    list.sort(Uniq_a)
    N_uniq_a = len(Uniq_a)
    Atom_freq_a = dict(Counter(a_labels))

    b_labels, b_coords, NA_b, junk = _sort_xyz(
        geometry=structure2.geometry, symbols=structure2.symbols)
   ...
   """
    if (NA_a == NA_b) & (Uniq_a == Uniq_b) & (Atom_freq_a == Atom_freq_b):
        ...
        # del Atom_freq['H']
        # print Atom_freq
        Sorted_Atom_freq = sorted(
            list(Atom_freq.items()), key=operator.itemgetter(1), reverse=True)
    ...
    if distance_metric == "GEODESIC":
        ...
        InitRMSD_sorted = ch.calculate_geodesic_xtb_barrier(node1, node2)
   ...
   """
    ...
    for i in range(len(Uniq)):
        ...
        vars()[a_Coords[Uniq[i]]] = parse_for_atom(
            a_labels, a_coords, str(Uniq[i]))
        vars()[a_Indices[Uniq[i]]] = get_atom_indices(a_labels, str(Uniq[i]))
        # print Uniq[i]
        # print vars()[a_Coords[Uniq[i]]]
        vars()[b_Coords[Uniq[i]]] = parse_for_atom(
            b_labels, b_coords, str(Uniq[i]))
   ...
   '''
    ...
    for i in range(len(B_t)):
        ...
        LAP = linear_sum_assignment(cost_matrix)
        vars()[Perm[Uniq[l]]] = []
        for j in range(len(LAP[0])):
            vars()[Perm[Uniq[l]]] += [(j, LAP[0][j])]
        vars()[Perm[Uniq[l]]] = sorted(
            vars()[Perm[Uniq[l]]], key=lambda x: x[0])
        ...
        if num_uniq == 1:
            ...
            if distance_metric == "GEODESIC":
                ...
                node2 = StructureNode(structure=Structure(
                    geometry=b_final, symbols=b_labels))
                rmsds.append([ch.calculate_geodesic_xtb_barrier(node1, node2), B_t[i][1],
                              B_t[i][2], b_final, vars()[Perm[Uniq[l]]]])
            ...
            while l < num_uniq:
                if l > 0:
                    vars()[b_Coords[Uniq[l]]] = parse_for_atom(
                        b_labels, b_final, Uniq[l])
                else:
                    vars()[b_Coords[Uniq[l]]] = parse_for_atom(
                        b_labels, b_trans, Uniq[l])
                # cost_matrix = np.array([[np.linalg.norm(a - b) for b in np.array(vars()[b_Coords[Uniq[l]]])]
                #                         for a in np.array(vars()[a_Coords[Uniq[l]]])])
                a_coords_array = np.array(vars().get(a_Coords[Uniq[l]], []))
                b_coords_array = np.array(vars().get(b_Coords[Uniq[l]], []))

                # Compute the cost matrix using np.linalg.norm for each pair (a, b)
                cost_matrix = np.array(
                    [[np.linalg.norm(a - b) for b in b_coords_array] for a in a_coords_array])
                LAP = linear_sum_assignment(cost_matrix)
                vars()[Perm[Uniq[l]]] = []
                ...
                vars()[Perm[Uniq[l]]] = [x[1] for x in vars()[Perm[Uniq[l]]]]
                # print str(vars()[b_Indices[Uniq[l]]])
                b_final = permute_atoms(
                    b_trans, vars()[Perm[Uniq[l]]], vars()[b_Indices[Uniq[l]]])
                b_trans = b_final
                l += 1
                q = l - 1
                if distance_metric == "GEODESIC":
                    ...
                    node2 = StructureNode(structure=Structure(
                        geometry=b_final, symbols=b_labels))
                    dist = ch.calculate_geodesic_distance(
                        node1, node2)
                    ...
                    node2 = StructureNode(structure=Structure(
                        geometry=b_final, symbols=b_labels))
                    dist = ch.calculate_geodesic_xtb_barrier(
                        node1, node2)

                elif distance_metric == "RMSD":
                    dist = kabsch(a_coords, b_final)
    ...
    if FinalRMSD < float(InitRMSD_unsorted):
        print(f"Initial unsorted {dist_name}: %2.3f" %
              float(InitRMSD_unsorted))
        print(f"Initial   sorted {dist_name}: %2.3f" % float(InitRMSD_sorted))
        print(f"Best             {dist_name}: %2.3f" % float(rmsds[0][0]))
    else:
        print("The initial alignment is already optimal.")
        print(f"Initial and final {dist_name}: %2.3f" %
              float(InitRMSD_unsorted))

    if FinalRMSD < float(InitRMSD_unsorted):
        b_final_labels, b_final_coords = permute_all_atoms(
            b_labels, rmsds[0][3], order)
    else:
...
def _sort_xyz(geometry, symbols):
    ...
    for i in range(num_atoms):
        ...
        sortedlines.append([symbols[i].upper(), float(
            geometry[i][0]), geometry[i][1], geometry[i][2], atomcount])
        atomcount += 1
    ...
    for i in range(atomcount):
        ...
        sortedcoords.append([float(sortedlines[i][1]), float(
            sortedlines[i][2]), float(sortedlines[i][3])])
...
def sorted_xyz(filename, noHydrogens):
    ...
    for i in range(num_atoms):
        ...
        if noHydrogens and line[0].upper().startswith("H"):
            ...
            sortedlines.append([line[0].upper(), float(
                line[1]), float(line[2]), float(line[3]), atomcount])
            atomcount += 1
    xyz.close()
...
"""
    ...
    parser.add_argument(
        '-z', '--zero', action='store_false', help='does nothing')
   ...
   """
    if InitRMSD_unsorted < 0.001:
        ...
        name = str(args.xyz2.split(".xyz")[0]) + \
            "-aligned_to-" + str(args.xyz1)
        write_to_xyz(num_atoms, name, b_labels, b_coords)
        print("Best alignment of " + str(args.xyz2) + " on " +
              str(args.xyz1) + " is written to " + str(name))
        sys.exit()
    ...
    if args.noHydrogens:
        ...
        print("Coordinates of " + str(args.xyz1) +
              " without hydrogens is written to " + str(name))
   ...
   '''
    ...
    print("Best alignment of " + str(args.xyz2) + " with " +
          str(args.xyz1) + " is written to " + str(name))

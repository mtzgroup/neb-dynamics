...
def main():
    ...
    ps.add_argument("--reconstruct", action="store_true", help="Reconstruct pair list at every iteration.")
    ...
    ps.add_argument("--nudge", dest="nudge", default=0.1, type=float, help="Random kick to the initial guess "
                    "path to allow finding multiple paths.")
    ps.add_argument("--ntries", dest="ntries", default=1, type=int, help="Number of trials to make when adding "
                    "new images.")
    args = ps.parse_args()
    ...
    # images are given
    raw = redistribute(symbols, X, args.nimages, tol=args.tol * 5, nudge=args.nudge, ntries=args.ntries)
    ...
    try:
        if args.sweep:
            smoother.sweep(tol=args.tol, max_iter=args.maxiter, micro_iter=args.microiter, reconstruct=args.reconstruct)
        else:

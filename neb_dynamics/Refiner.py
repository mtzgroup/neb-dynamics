...
class Refiner:
    ...
    def __post_init__(self):
        ...
        if self.optimizer is None:
            self.optimizer = VelocityProjectedOptimizer(timestep=1.0, activation_tol=0.1)
    ...
    def refine_xtb_chain(self, xtb_chain):
        ...
        m = MSMEP(neb_inputs=self.nbi, chain_inputs=self.cni, gi_inputs=self.gii, 
    ...
    def join_output_leaves(self, refined_leaves):
        joined_nodes = []
        [joined_nodes.extend(leaf.output_chain.nodes) for leaf in refined_leaves if leaf]
    ...
    def read_leaves_from_disk(self, out_directory):
        ...
        if self.v: print('opt steps:',sum(num_steps))

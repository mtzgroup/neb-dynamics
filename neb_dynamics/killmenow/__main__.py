...
def main():
    ...
    ps = argparse.ArgumentParser(
        description="Interpolates between two geometries",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ps.add_argument(
        "filename",
        type=str,
        help="XYZ file containing geometries. If the number of images "
    ...
    ps.add_argument(
        "--sweep",
        action="store_true",
        help="Sweep across the path optimizing one image at "
    ...
    ps.add_argument(
        ...
        dest="sweep",
        action="store_false",
        help="Do not perform sweeping.",
    )
    ps.set_defaults(sweep=None)
    ps.add_argument(
        "--output",
        default="interpolated.xyz",
        type=str,
        help="Output filename. " "Default is interp.xyz",
    )
    ...
    ps.add_argument(
        ...
        default=20,
        type=int,
        help="Maximum number of micro iterations for " "sweeping algorithm.",
    )
    ps.add_argument(
        "--scaling",
        default=1.7,
        type=float,
        help="Exponential parameter for morse potential",
    )
    ps.add_argument(
        "--friction",
        default=1e-2,
        type=float,
        help="Size of friction term used to prevent " "very large change of geometry.",
    )
    ps.add_argument(
        "--dist-cutoff",
        dest="dist_cutoff",
    ...
    ps.add_argument(
        "--reconstruct",
        action="store_true",
        help="Reconstruct pair list at every iteration.",
    )
    ps.add_argument(
        "--save-raw",
        dest="save_raw",
    ...
    ps.add_argument(
        ...
        default=0.1,
        type=float,
        help="Random kick to the initial guess "
        "path to allow finding multiple paths.",
    )
    ps.add_argument(
        "--ntries",
        dest="ntries",
        default=1,
        type=int,
        help="Number of trials to make when adding " "new images.",
    )
    args = ps.parse_args()
    ...
    # images are given
    raw = redistribute(
        symbols, X, args.nimages, tol=args.tol * 5, nudge=args.nudge, ntries=args.ntries
    )
    ...
    smoother = Geodesic(
        symbols, raw, args.scaling, threshold=args.dist_cutoff, friction=args.friction
    )
